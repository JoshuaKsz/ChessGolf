export class DrawLib {
    ...
    // Function to draw a chessboard pattern
  draw_chessboard(squareSize, rows, cols, color1, color2) {
    // Draw grid lines
    for (let i = 0; i <= rows; i++) {
      this.create_line(0, i * squareSize, cols * squareSize, i * squareSize, { r: 0, g: 0, b: 0 });
    }
    for (let j = 0; j <= cols; j++) {
      this.create_line(j * squareSize, 0, j * squareSize, rows * squareSize, { r: 0, g: 0, b: 0 });
    }
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const fillColor = isColor1 ? color1 : color2;
